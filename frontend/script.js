...
const API_BASE = 'https://pretty-gratitude-production-9606.up.railway.app';

/**
 * fetch() wrapper that always sends the PHP session cookie.
 * Required because the frontend (Vercel) and the backend (Railway)
 * are on different domains — without `credentials:'include'` the
 * CSRF token issued by api/csrf.php would never match the session
 * insert.php sees, and every submission would fail with a 403.
 */
...
function initForm() {
  ...
  function fetchCsrfToken() {
    apiFetch(`${API_BASE}/api/csrf.php`)
      .then((r) => r.json())
      .then((data) => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/insert.php`, { method: 'POST', body: formData });
